...
from converter import to_bin, to_hex
from translator import get_words
from number_image_creator import create, add_number
import numpy as np
...
def generate(number, def_colors):
    number -= 1
    image_path = create(NUMBERS[number][0], (255, 255, 255), def_colors[number][1])
    font = "../fonts/arial-unicode-ms.ttf"
    result_path = "../output/words/" + NUMBERS[number][0] + ".png"

    word_list = get_words(NUMBERS[number][1])
    word_list[to_bin(NUMBERS[number][0])] = 1
    word_list[to_hex(NUMBERS[number][0])] = 1
    print(word_list)

    bg = np.array(Image.open(image_path))
    wc = WordCloud(
        mask=bg,
        width=1024,
        height=1024,
        background_color=def_colors[number][1],
        max_font_size=150,
        min_font_size=15,
        max_words=5000,
        font_path=font,
    ...
    add_number(NUMBERS[number][0], def_colors[number][0])
...
def loop_generate():
    ...
    for j in range(151, n):
        is_done = False
        while not is_done:
            try:
                print("Generowanie obrazu nr: " + str(j + 1))
                generate(j + 1, colors)
                is_done = True
            except IOError as e:
                print(e)
